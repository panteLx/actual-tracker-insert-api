...
export const limiter = rateLimit({
  ...
  max: config.NODE_ENV === "production" ? 100 : 1000, // Limit each IP to 100 requests per minute
...
export const transactionLimiter = rateLimit({
  ...
  max: config.NODE_ENV === "production" ? 3 : 1000, // Limit each IP to 3 transaction request per minute
  handler: (req, res) => {
